...
export async function handleJoinCall(
  ...
  const isVoiceChannelSession = Boolean(session.channel_id);
  ...
  if (participantId && !tokenAuthorized) {
    ...
    if (inviteStatus === "pending" && !isVoiceChannelSession) {
  ...
  } else if (isVoiceChannelSession && joinedCount >= 1 && sessionStatus !== "active") {
    sessionUpdates.status = "active";
    sessionUpdates.started_at = session.started_at ?? nowIso;
